...
export default function WeatherApp() {
    ...
    try {
      const response = await fetch(
          `https://api.geoapify.com/v1/geocode/autocomplete?text=${encodeURIComponent(
              query
          )}&format=json&apiKey=2cb1781c0583403d8f80e2299027fbec`
      )
  ...
  const fetchWeather = async (lat: number, lon: number) => {
    ...
    try {
      const response = await fetch(
          `https://api.openweathermap.org/data/3.0/onecall?lat=${lat}&lon=${lon}&exclude=minutely,alerts&units=metric&appid=7c7cfafd447ad3608db25da723ffe1d2`
      )
      const data = await response.json()
      setWeatherData(data)
    } catch (error) {
